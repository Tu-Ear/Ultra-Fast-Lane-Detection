...
void help(void)
{
	...
	cout << "-m                  : save visualization result" << endl;
	cout << "-b                  : nums of bad image to save" << endl;
}
...
void save_visualize(string &save_visualize_dir, string &full_im_name, vector<vector<Point2f>> &anno_lanes, vector<vector<Point2f>> &detect_lanes, vector<int> anno_match, int width_lane);
...
int main(int argc, char **argv)
{
	...
	bool save = false;
	int frame = 1;
	while ((oc = getopt(argc, argv, "ha:d:i:l:w:t:c:r:smf:o:b:")) != -1)
	{
		switch (oc)
		{
		case 'h':
...
/*
    anno_dir: /data/ldp/zjf/dataset/CULane/
    detect_dir: /data/ldp/zjf/evaluation/Debug/Official/culane_18.pth/culane_eval_tmp/
    output_file: /data/ldp/zjf/evaluation/Debug/Official/culane_18.pth/txt/out1_crowd.txt
    im_dir: /data/ldp/zjf/dataset/CULane/
    list_im_file: /data/ldp/zjf/dataset/CULane/list/test_split/test1_crowd.txt
    width_lane: 30
    iou_threshold: 0.5
    im_width: 1640
    im_height: 590
    show visualization: 0
...
*/
	...
	cout << "show visualization: " << show << endl;
	cout << "save visualization: " << save << endl;
	...
	// eval_dir: /data/ldp/zjf/evaluation/Debug/Official/culane_18.pth
    string eval_dir = output_file.substr(0, output_file.find("/txt"));
...
#pragma omp parallel for
	...
	/*
        im_dir: /data/ldp/zjf/dataset/CULane/
        sub_im_name: driver_100_30frame/05250650_0337.MP4/02520.jpg
        full_im_name: /data/ldp/zjf/dataset/CULane/driver_100_30frame/05250650_0337.MP4/02520.jpg
        anno_dir: /data/ldp/zjf/dataset/CULane/
        detect_dir: /data/ldp/zjf/evaluation/Debug/Official/culane_18.pth/culane_eval_tmp/
        sub_txt_name: driver_100_30frame/05250650_0337.MP4/02520.lines.txt
        anno_file_name: /data/ldp/zjf/dataset/CULane/driver_100_30frame/05250650_0337.MP4/02520.lines.txt
        detect_file_name: /data/ldp/zjf/evaluation/Debug/Official/culane_18.pth/culane_eval_tmp/driver_100_30frame/05250650_0337.MP4/02520.lines.txt
        ------------------------------------------------------------------
        cout << "im_dir: " << im_dir << endl;
    ...
    */
		...
		/*
		    eval_dir: /data/ldp/zjf/evaluation/Debug/Official/culane_18.pth
		    ------
            cout << "eval_dir: " << eval_dir << endl;
        */
            string save_visualize_dir = eval_dir + "/show";
			auto anno_match = get<0>(tuple_lists[i]);
			save_visualize(save_visualize_dir, full_im_name, anno_lanes, detect_lanes, anno_match, width_lane);
		}
	}
	...
	ofs_out_file << "Fmeasure: " << F << endl;
	ofs_out_file << "Similarity of each image: " << endl;
	...
	for (auto result : score_lists)
	{
		...
		ofs_out_file << left << setw(50) << score_img.substr(score_img.find("driver")) + ": ";
	...
	if (num_bad != 0)
	{
		...
		int type_length = output_file.find(".txt") - type_pos;

    /*
        bad_img_dir: /data/ldp/zjf/evaluation/Debug/Official/culane_18.pth/bad
        type: /out1_crowd
        bad_type_img_dir: /data/ldp/zjf/evaluation/Debug/Official/culane_18.pth/bad/out1_crowd
        -----------
		cout << "bad_img_dir: " << bad_img_dir << endl;
		cout << "type: " << type << endl;
		cout << "bad_type_img_dir: " << bad_type_img_dir << endl;
    */
		string bad_img_dir = eval_dir + "/bad";
		string type = output_file.substr(type_pos, type_length);
		string bad_type_img_dir = bad_img_dir + type;

		makedir(bad_img_dir);
		makedir(bad_type_img_dir);

		int len = score_lists.size();

        for (int i = len - 1; i > len - 1 - num_bad; i--)
        {
        /*
            len: 8113
            i: 8108
            bad_img: /data/ldp/zjf/dataset/CULane/driver_193_90frame/06061004_0835.MP4/01080.jpg
            dest: /data/ldp/zjf/evaluation/Debug/Official/culane_18.pth/bad/5_01080.jpg
            -------------------------------
            cout << "len: " << len << endl;
            cout << "i: " << i << endl;
            cout << "bad_img: " << bad_img << endl;
            cout << "dest: " << dest << endl;
            cout << "------" << endl;
        */
            string bad_img = get<0>(score_lists.at(i));
            string dest = bad_type_img_dir + "/" + to_string(len - i) + "_" + bad_img.substr(bad_img.find("MP4") + 4);
            ...
            string sub_im_name = bad_img.substr(bad_img.find("driver"));
            string full_im_name = im_dir + sub_im_name;
            string sub_txt_name = sub_im_name.substr(0, sub_im_name.find_last_of(".")) + ".lines.txt";
            string anno_file_name = anno_dir + sub_txt_name;
            string detect_file_name = detect_dir + sub_txt_name;
...
//		save_bad(im_dir, anno_dir, detect_dir, bad_type_img_dir, score_lists, num_bad);
	}
...
void save_visualize(string &save_visualize_dir, string &full_im_name, vector<vector<Point2f>> &anno_lanes, vector<vector<Point2f>> &detect_lanes, vector<int> anno_match, int width_lane)
{
	...
	//--------------------------------------------------------
	string save_path = save_visualize_dir + "/img";
	string save_path2 = save_visualize_dir + "/img2";
    cout<<"save_path: "<<save_path<<endl;
    cout<<"save_path2: "<<save_path2<<endl;

	makedir(save_visualize_dir);
	makedir(save_path);
	makedir(save_path2);
	//--------------------------------------------------------
	/*
...
void save_bad(string &im_dir, string &anno_dir, string &detect_dir, string &bad_img_dir, vector<tuple<string, double>> &score_lists, int num)
{
	...
	for (int i = len - 1; i > len - 1 - num; i--)
	{
		...
		*/
		...
		auto anno_match = get<0>(tuple_list);
		save_visualize(full_im_name, full_im_name, anno_lanes, detect_lanes, anno_match, 30);
	}
}